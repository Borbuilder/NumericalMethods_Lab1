...
import numpy as np
from TestTaskModule import TestTask  # Import TestTask class from C++
...
class Window:
    ...
    def create_first_task_widgets(self):
        # Here you can add input fields and logic for the first task
        tk.Label(self.master, text="Это интерфейс для Первой задачи (реализация в процессе)").grid(row=2, columnspan=2)
    ...
    def calculate_values(self):
        try:
            ...
            if self.button_mode.cget('text') == 'С ОЛП':
                ...
                self.update_table(task.get_table_information(), 0)
            else:
                task.Solve_Without_Error_Control()
                self.update_table(task.get_table_information(), 1)
    ...
    def update_table(self, table_data, olp=0):
